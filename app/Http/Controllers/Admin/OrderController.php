...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($startDate && !$endDate) {
            Session::flash('swalerror', 'The end date is required if the start date is present');
            return redirect('admin/orders');
        }

        if (!$startDate && $endDate) {
            Session::flash('swalerror', 'The start date is required if the end date is present');
            return redirect('admin/orders');
        }

        if ($startDate && $endDate) {
            if (strtotime($endDate) < strtotime($startDate)) {
                Session::flash('swalerror', 'The end date should be greater or equal than start date');
                return redirect('admin/orders');
            }
    ...
    public function doCancel(Request $request, $id)
    {
        ...
        Session::flash('swalsuccess', 'The order has been cancelled');
    ...
    public function doComplete(Request $request, $id)
    {
        ...
        if (!$order->isDelivered()) {
            Session::flash('swalerror', 'Mark as complete the order can be done if the latest status is delivered');
            return redirect('admin/orders');
        }
        ...
        if ($order->save()) {
            Session::flash('swalsuccess', 'The order has been marked as completed!');
            return redirect('admin/orders');
        }
    }
    ...
    public function destroy($id)
    {
        ...
        if ($order->trashed()) {
            ...
            if ($canDestroy) {
                Session::flash('swalsukses', 'The order has been removed permanently');
            } else {
                Session::flash('swalsukses', 'The order could not be removed permanently');
            }
            ...
            if ($canDestroy) {
                Session::flash('swalsukses', 'The order has been removed');
            } else {
                Session::flash('swalsukses', 'The order could not be removed');
            }
    ...
    public function restore($id)
    {
        ...
        if ($canRestore) {
            Session::flash('swalsukses', 'The order has been restored');
            return redirect('admin/orders');
        } else {
            if (!Session::has('error')) {
                Session::flash('swalerror', 'The order could not be restored');
            }
            return redirect('admin/orders/trashed');
        }
    }
